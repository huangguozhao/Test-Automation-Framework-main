...
@allure.epic("EasyJava系统API测试")
@allure.feature("系统配置管理")
class TestSystemConfigManagement:
    """
    系统配置管理模块测试 - 使用YAML数据驱动
    """
    
    @pytest.mark.parametrize("case_data", get_testcase_yaml("testcase/easyjava_test/system_config.yaml"))
    def test_system_config_api(self, case_data):
        """
        系统配置管理接口测试
        """
        # 检查数据结构并正确获取测试用例信息
    ...
    def _run_test_case(self, base_info, test_case):
        ...
        with allure.step("请求信息"):
            ...
            if test_case.get("params"):
                allure.attach(str(test_case["params"]), "请求参数", allure.attachment_type.TEXT)
        ...
        with allure.step("执行断言验证"):
            ...
            assertions.assert_result(test_case.get("validation", []), response_json, response.status_code)
